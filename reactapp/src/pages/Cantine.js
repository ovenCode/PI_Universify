...
const Cantine = ({ userId, mainContent }) => {
    ...
    return (
        <div>
            <div id="cantine-page">
                <div id="cantine" className="mainContent">
                    ...
                    <div id="cantine-info">
                        <div style={{ gridColumn: 1, gridRow: 1 }}>Godziny otwarcia:</div>
                        <div style={{ gridColumn: 1, gridRow: 2 }}>{cantine.hours}</div>
                        <div style={{ gridColumn: 2, gridRow: 1 }}>Lokalizacja:</div>
                        <div style={{ gridColumn: 2, gridRow: 2 }}>{cantine.address.name}</div>
                        <div style={{ gridColumn: 3, gridRow: 1 }}>Piętro:</div>
                        <div style={{ gridColumn: 3, gridRow: 2 }}>{cantine.address.floor}</div>
                    </div>
...
const loadCantine = async ({ ignore, userId, cantine, setCantine }) => {
    ...
    if (!ignore) {
        ...
        cantine.skip = Object.keys(answer).length !== 0 ? true : false;
...
const AddOrder = ({ setPopup, course, setCourse, data }) => {
    ...
    // FIXME: Fix the select to show selected meal and load said meal into the dialog
        ...
        <div id="order-add-dialog" className="dialog">
            ...
            <div id="dialog-btns">
                <div className="btn-dark" id="add-course-btn" onClick={async () => {
                    // TODO: Add confirmation so that user knows order has been successfully added
                    const id = selected;
                    const date = new Date();
                    var object = {
                        "nazwa": course.nazwa,
